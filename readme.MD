...
- 수정 및 추가작업
  ...
  - git 관련 id 조회
    ```
      ...
      # ID 조회
      gh api repos/jaeyan42/devops_tf_k8s_ci --jq "{owner_id: .owner.id, repository_id: .id, created_at: .created_at}"
      ---
      {
...
- ci 셋업 1회 진행
```
./setup-ci.bat jaeyan42/devops_tf_k8s_ci 
./setup-ci.sh jaeyan42/devops_tf_k8s_ci
```
